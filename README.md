...
2. Checks for speeches/announcements by **Andrej Karpathy, Jensen Huang, Alex Karp, Sam Altman, Elon Musk, Donald Trump** — only names with a market-moving remark are included
...
| **Speeches & announcements** _(email)_                    | Google News RSS — last 24h per person (up to 24 headlines; `when:1d`)                                                                                              | Core brief: reviews all items, keeps **one** market/crypto-moving statement (or omits the name) + optional **quote** + source   |
